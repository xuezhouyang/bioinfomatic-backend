#!/usr/bin/env bash
# =============================================================================
# Layer 1: Setup — Create worktree, branch, and initialize tracking state
# =============================================================================
# Usage: setup.sh <task_id> <branch> <worktree_dir> [issue_body]
# =============================================================================
...
# Arguments
# ---------------------------------------------------------------------------
TASK_ID="${1:?Usage: setup.sh <task_id> <branch> <worktree_dir> [issue_body]}"
BRANCH="${2:?Usage: setup.sh <task_id> <branch> <worktree_dir> [issue_body]}"
WORKTREE_DIR="${3:?Usage: setup.sh <task_id> <branch> <worktree_dir> [issue_body]}"
ISSUE_BODY="${4:-No description provided}"
...
if [ -d "$WORKTREE_DIR" ]; then
    ...
    git worktree add "$WORKTREE_DIR" "$BRANCH" 2>/dev/null || {
        ...
        git checkout "$BRANCH"
        # Point worktree dir to repo root as fallback
...
# ---------------------------------------------------------------------------
# Extract issue title from task_id (best-effort)
TASK_TITLE="${TASK_ID}"
...
if command -v jq &>/dev/null; then
    ...
    echo "[setup] WARNING: jq not installed. Writing minimal active-tasks.json."
    # Append manually (not ideal but functional)
    python3 -c "
import json, sys
...
" 2>/dev/null || echo "[setup] WARNING: Could not update active-tasks.json (no jq or python3)."
fi

# ---------------------------------------------------------------------------
# 5. Write daily memory entry
# ---------------------------------------------------------------------------
...
echo "- **${TIME_NOW}** [${TASK_ID}] Task initialized — branch: ${BRANCH}" >> "$DAILY_FILE"

# ---------------------------------------------------------------------------
# 6. Ensure .gitignore excludes runtime state
# ---------------------------------------------------------------------------
...
for PATTERN in ".clawdbot/active-tasks.json" "memory/" ".clawdbot/*.pid"; do
